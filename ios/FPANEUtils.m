...
void AirAlert_FPANE_Log(FREContext context, NSString* message) {
    ...
    AirAlert_FPANE_DispatchEventWithInfo(context, @"LOGGING", message);
}
...
NSArray* AirAlert_FPANE_FREObjectToNSArrayOfUIImage(FREObject object) {
    ...
    for (NSInteger i = 0; i < arrayLength; i++) {
        ...
        UIImage *image = AirAlert_FPANE_FREBitmapDataToUIImage(itemRaw);
...
FREObject AirAlert_FPANE_CreateError(NSString* error, NSInteger* id) {
    ...
    FREObject argV[] = {
            AirAlert_FPANE_NSStringToFREObject(error),
            freId
    };
